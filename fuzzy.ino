...
String getFuzzyMoistureLevel(double soilMoisture) {
  if (soilMoisture < 70) {
    return "dry";
...
String getFuzzyTemperatureLevel(float temperature) {
  if (temperature < 18) {
    return "cold";
...
int fuzzyInference(String moistureLevel, String temperatureLevel) {
  if (moistureLevel == "dry" && temperatureLevel == "hot") {
    return 2500; // Long duration (2.5 seconds)
  } else if (moistureLevel == "dry" && temperatureLevel == "moderate") {
    return 1500; // Moderate duration (1.5 seconds)
  } else if (moistureLevel == "ideal") {
    return 1000; // Short duration (1 second)
  } else {
    return 0; // Pump off
  }
}

void controlPump(int pumpDuration) {
  if (pumpDuration > 0) {
    digitalWrite(relayPin, HIGH);
    delay(pumpDuration); // Operate the pump for the specified duration
    digitalWrite(relayPin, LOW);
    pumpTotalTime += pumpDuration; // Accumulate pump run time
    wateringCount++;
  }
}
...
void loop() {
  ...
  if (currentTime - startTime <= monitoringDuration) {
    ...
    String moistureLevel = getFuzzyMoistureLevel(soilMoisture);
    String temperatureLevel = getFuzzyTemperatureLevel(temperature);
    int pumpDuration = fuzzyInference(moistureLevel, temperatureLevel);
    ...
    Serial.println("\n=== Monitoring Completed Fuzzy (30 seconds) ===");
